...
export class CourseScheduleAgent {
  constructor(private readonly geminiAgent: GeminiAiAgent) {}
  ...
  private generatePrompt(data: CourseScheduleAssistantProps) {
    const { semesters, shifts = '1', periodsLength = 45 } = data;
    const allTeachers = data.teachers || [];
    const allCourse = data.courses || [];
    const allTeachersUnavailability = data.teachersUnavailability || [];
